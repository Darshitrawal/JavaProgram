...
public class Transaction{
  ...
  // String[] streamsTitle = new String[6];
  // String[] streamsLocation = new String[6];
  int i , j, k, balance;
  ...
  throws InsufficientBalanceException{
    if(userType.equals("student")){
      if(DAO.students.get(userFoundIndex).bookLimit > 4){
      ...
      if(DAO.staffs.get(userFoundIndex).bookLimit > 4){
        msg = "you didn't Issued any book!";
        throw new InsufficientBalanceException(msg);
      }else{
        DAO.staffs.get(userFoundIndex).bookLimit++;
  ...
  public boolean newUser(String newUserId, String newUserEnroll, String newUserName, String newUserBrach, String newUserSem){
    int newUserIdInt = Integer.parseInt(newUserId);
    int newUserEnrollInt = Integer.parseInt(newUserId);
    int newUserSemInt = Integer.parseInt(newUserId);
    if(DAO.students.add(new Student(newUserIdInt, newUserEnrollInt, newUserName, newUserBrach, newUserSemInt))){
      status = true;
    }else{
      status = false;
    }
  ...
  //Overloaded
  public boolean newUser(String newUserId, String newUserName, String newUserBrach, String newUserDesignation){
    int newUserIdInt = Integer.parseInt(newUserId);
    if(DAO.staffs.add(new Staff(newUserIdInt, newUserName, newUserBrach, newUserDesignation))){
      status = true;
    }else{
      status = false;
    }
